...
func is_num_char(tls *libc.TLS, c int32_t) (r uint8) {
	return libc.BoolUint8(c == int32('_') || iswdigit(tls, c) != 0)
}
...
func tree_sitter_ron_external_scanner_scan(tls *libc.TLS, payload uintptr, lexer uintptr, valid_symbols uintptr) (r uint8) {
	...
	if *(*uint8)(unsafe.Pointer(valid_symbols + uintptr(FLOAT))) != 0 && iswdigit(tls, (*TSLexer)(unsafe.Pointer(lexer)).Flookahead) != 0 {
		...
		if (*TSLexer)(unsafe.Pointer(lexer)).Flookahead == int32('.') {
			...
			advance(tls, lexer)
			if iswalpha(tls, (*TSLexer)(unsafe.Pointer(lexer)).Flookahead) != 0 {
				return libc.BoolUint8(0 != 0)
			}
			if (*TSLexer)(unsafe.Pointer(lexer)).Flookahead == int32('.') {
				return libc.BoolUint8(0 != 0)
		...
		advance(tls, lexer)
		if !(iswdigit(tls, (*TSLexer)(unsafe.Pointer(lexer)).Flookahead) != 0) {
			return libc.BoolUint8(1 != 0)
		}
		for iswdigit(tls, (*TSLexer)(unsafe.Pointer(lexer)).Flookahead) != 0 {
			advance(tls, lexer)
		}
		(*(*func(*libc.TLS, uintptr))(unsafe.Pointer(&struct{ uintptr }{(*TSLexer)(unsafe.Pointer(lexer)).Fmark_end})))(tls, lexer)
		return libc.BoolUint8(1 != 0)
